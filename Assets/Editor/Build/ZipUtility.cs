...
public static class ZipUtility
{
    ...
    /// 解压文件
    /// </summary>
    /// <param name="targetPath">解压位置</param>
    /// <param name="zipFilePath">zip压缩文件目录(包括文件名和后缀)</param>
    public static void DecompressToDirectory(string targetPath, string zipFilePath, Action callback = null)
    {
        if (File.Exists(zipFilePath))
        {
            var compressed = File.OpenRead(zipFilePath);
            compressed.DecompressToDirectory(targetPath, callback);
        }
        else
        {
    ...
    public static void DecompressToDirectory(this Stream source, string targetPath, Action callback = null)
    {
