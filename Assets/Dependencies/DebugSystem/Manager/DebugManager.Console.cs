...
namespace DebugSystem
{
    internal partial class DebugManager
    {
        ...
        private void DebugConsole()
        {
            ...
            if (GUILayout.Button($"[{logInfo}]", DebugData.Button, DebugData.Height))
            {
                showInfo = !showInfo;
            }
            
            GUI.contentColor = showWarn ? Color.white : Color.gray;
            if (GUILayout.Button($"[{logWarn}]", DebugData.Button, DebugData.Height))
            {
                showWarn = !showWarn;
            }

            GUI.contentColor = showError ? Color.white : Color.gray;
            if (GUILayout.Button($"[{logError}]", DebugData.Button, DebugData.Height))
            {
                showError = !showError;
            }

            GUI.contentColor = showException ? Color.white : Color.gray;
            if (GUILayout.Button($"[{logException}]", DebugData.Button, DebugData.Height))
            {
            ...
            for (int i = 0; i < logList.Count; i++)
            {
                ...
                if (show)
                {
                    ...
                    if (GUILayout.Toggle(logIndex == i, logList[i].showTitle, DebugData.HeightLow)) // DebugData.Label))
                    {
                        logIndex = i;
                    }
            ...
            if (logIndex != -1)
            {
                ...
                if (GUILayout.Button("COPY", GUILayout.Width(60f), GUILayout.Height(30f)))
                {
                    ...
                    textEditor.text = string.Format("{0}\n\n{1}", logList[logIndex].message, logList[logIndex].stackTrace);
                    textEditor.OnFocus();
                    textEditor.Copy();
                }
                GUILayout.EndHorizontal();
