...
namespace FSG.MeshAnimator
{
    public class MeshAnimatorManager : MonoBehaviour
    {
        ...
        private static HashSet<IMeshAnimator> _animatorHashset = new HashSet<IMeshAnimator>();
        ...
        private void UpdateTick(float time)
        {
            ...
            if (_addAnimators.Count > 0)
            {
                ...
                _animatorHashset.UnionWith(_addAnimators);
                _addAnimators.Clear();
                count = _animators.Count;
            }
            ...
            if (_removeAnimators.Count > 0)
            {
                ...
                // }
                for (int i = _removeAnimators.Count - 1; i >= 0; i--)
                {
                    IMeshAnimator remove = _removeAnimators[i];
                    if (_animatorHashset.Remove(remove))
                    {
