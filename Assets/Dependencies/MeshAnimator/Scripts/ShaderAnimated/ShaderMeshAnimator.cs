...
namespace FSG.MeshAnimator.ShaderAnimated
{
    ...
    public class ShaderMeshAnimator : MeshAnimatorBase
    {
        ...
        private List<Material> _materialCacheLookup = new List<Material>();  //2024.01.17�Ż�
        ...
        public void ClearAllMaterial()
        {
            for (int i = 0; i < _materialCacheLookup.Count; i++)
            {
                var mat = _materialCacheLookup[i];
                _setMaterials.Remove(mat);
            }
        }

        public void ReloadMaterial(bool clearcache)
        {
            if (clearcache)
                ClearAllMaterial();
            SetupTextureData();
