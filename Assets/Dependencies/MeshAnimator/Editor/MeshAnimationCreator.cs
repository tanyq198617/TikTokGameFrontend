    ...
    public class MeshAnimationCreator : EditorWindow
    {
        ...
        private void OnGUI()
        {
            ...
            if (prefab != null && !string.IsNullOrEmpty(GetPrefabPath()))
            {
                ...
                GUILayout.BeginHorizontal();
                {
                    isRotation = EditorGUILayout.Toggle("自定义旋转", isRotation, GUILayout.MaxWidth(50));
                    if (isRotation)
                    {
                        GUILayout.Space(140);
                        rotationX = EditorGUILayout.Toggle(rotationX, GUILayout.MaxWidth(10));
                        GUILayout.Label("X轴", GUILayout.MaxWidth(30));
                        if (rotationX) { rotationY = false; rotationZ = false; }

                        rotationY = EditorGUILayout.Toggle(rotationY, GUILayout.MaxWidth(10));
                        GUILayout.Label("Y轴", GUILayout.MaxWidth(30));
                        if (rotationY) { rotationX = false; rotationZ = false; }
                        
                        rotationZ = EditorGUILayout.Toggle(rotationZ, GUILayout.MaxWidth(10));
                        GUILayout.Label("Z轴", GUILayout.MaxWidth(30));
                        if (rotationZ) { rotationX = false; rotationY = false; }
                        
                        GUILayout.Label("旋转度数:", GUILayout.MaxWidth(60));
                        rotationAngle = EditorGUILayout.IntField(rotationAngle, GUILayout.MaxWidth(40));
                    }
                }
                GUILayout.EndHorizontal();
            }
        ...
        {
            // 2023.12.29 按X轴旋转顶点
            if (isRotation && rotationAngle != 0)
            {
                // Create a quaternion for the desired rotation
                var axis = Vector3.zero;// new Vector3(, rotationY, rotationZ);
                if (rotationX) axis = new Vector3(1, 0, 0);
                if (rotationY) axis = new Vector3(0, 1, 0);
                if (rotationZ) axis = new Vector3(0, 0, 1);
                var rotationQuaternion = Quaternion.AngleAxis(rotationAngle, axis.normalized);

                // Rotate vertices using the quaternion
                var rotatedVertices = new Vector3[from.vertices.Length];
                for (int i = 0; i < from.vertices.Length; i++)
                {
                    rotatedVertices[i] = rotationQuaternion * from.vertices[i];
                }

                // Transfer rotated vertices
                to.vertices = rotatedVertices;
            }
            else
            {
