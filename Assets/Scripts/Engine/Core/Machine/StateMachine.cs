using System;
using Spine.Collections;

/// <summary>
/// 状态机
/// </summary>
public class StateMachine
{
    private readonly OrderedDictionary<string, IStateNode> _nodes = new OrderedDictionary<string, IStateNode>(100);
    private IStateNode _curNode;
    private IStateNode _preNode;

    /// <summary>
    /// 状态机持有者
    /// </summary>
    public System.Object Owner { private set; get; }

    /// <summary>
    /// 当前运行的节点名称
    /// </summary>
    public string CurrentNode
    {
        get { return _curNode != null ? _curNode.GetType().FullName : string.Empty; }
    }

    /// <summary>
    /// 之前运行的节点名称
    /// </summary>
    public string PreviousNode
    {
        get { return _preNode != null ? _preNode.GetType().FullName : string.Empty; }
    }


    private StateMachine() { }
    public StateMachine(System.Object owner)
    {
        Owner = owner;
    }

    public void SysUpdate() => _curNode?.SysUpdate();
    public void FixedUpdate() => _curNode?.FixedUpdate();
    public void LateUpdate() => _curNode?.LateUpdate();

    /// <summary>
    /// 启动状态机
    /// </summary>
    public void Run<TNode>() where TNode : IStateNode
    {
        var nodeType = typeof(TNode);
        var nodeName = nodeType.FullName;
        Run(nodeName);
    }
    public void Run(Type entryNode)
    {
        var nodeName = entryNode.FullName;
        Run(nodeName);
    }
    public void Run(string entryNode)
    {
        _curNode = TryGetNode(entryNode);
        _preNode = _curNode;

        if (_curNode == null)
            throw new Exception($"无法找到状态机入口节点: {entryNode}");

        _curNode.OnEnter();
    }

    public bool RunNextNode<TNode>(TNode node) where TNode : IStateNode
    {
        var nodeType = typeof(TNode);
        var nodeName = nodeType.FullName;
        int index = _nodes.IndexOf(nodeName);
        if (index == -1 || index == _nodes.Count - 1)
            return false;
        string nextNode = _nodes.GetKey(index + 1);
        ChangeState(nextNode);
        return true;
    }

    private void NodeStartChange(int index, string nodeName)
        => EventMgr.Dispatch(GameEvent.NodeStartChange, index, _nodes.Count, nodeName);

    private void NodeChanged(int index, string nodeName)
        => EventMgr.Dispatch(GameEvent.NodeChanged, index, _nodes.Count, nodeName);

    /// <summary>
    /// 加入一个节点
    /// </summary>
    public TNode AddNode<TNode>() where TNode : IStateNode
    {
        var nodeType = typeof(TNode);
        var stateNode = Activator.CreateInstance(nodeType) as IStateNode;
        AddNode(stateNode);
        return (TNode)stateNode;
    }
    public void AddNode(IStateNode stateNode)
    {
        if (stateNode == null)
            throw new ArgumentNullException();

        var nodeType = stateNode.GetType();
        var nodeName = nodeType.FullName;

        if (_nodes.ContainsKey(nodeName) == false)
        {
            stateNode.OnCreate(this);
            _nodes.Add(nodeName, stateNode);
        }
        else
        {
            Debuger.LogError($"[状态机] 状态节点不存在: {nodeName}");
        }
    }

    /// <summary>
    /// 转换状态节点
    /// </summary>
    public void ChangeState<TNode>() where TNode : IStateNode
    {
        var nodeType = typeof(TNode);
        var nodeName = nodeType.FullName;
        ChangeState(nodeName);
    }
    public void ChangeState(Type nodeType)
    {
        var nodeName = nodeType.FullName;
        ChangeState(nodeName);
    }
    public void ChangeState(string nodeName)
    {
        if (string.IsNullOrEmpty(nodeName))
            throw new ArgumentNullException();

        IStateNode node = TryGetNode(nodeName);
        if (node == null)
        {
            Debuger.LogError($"[状态机] 找不到状态节点:  {nodeName} (极可能是没有注册)");
            return;
        }

        //Debuger.Log($"{_curNode.GetType().FullName} --> {node.GetType().FullName}");
        int index = _nodes.IndexOf(nodeName);
        _preNode = _curNode;
        _curNode.OnExit();
        NodeStartChange(index, nodeName);
        _curNode = node;
        _curNode.OnEnter();
        NodeChanged(index, nodeName);
    }

    private IStateNode TryGetNode(string nodeName)
    {
        _nodes.TryGetValue(nodeName, out IStateNode result);
        return result;
    }
    
    public bool InNode<TNode>() where TNode : IStateNode 
    {
        var nodeType = typeof(TNode);
        var nodeName = nodeType.FullName;
        return nodeName.Equals(CurrentNode);
    }
}
